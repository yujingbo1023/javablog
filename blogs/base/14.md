...
![1699924640575](./assets/1699924640575.png)

在Cart.vue中获取上面的list，如下：

![1699924813408](./assets/1699924813408.png)

使用给某个账号添加一些数据，如下：

![1699924860383](./assets/1699924860383.png)

![1699924908707](./assets/1699924908707.png)

购物车中的数据就准备好了。在商品详情中，点击了添加购物车，实际上调用了两个接口，一个是添加商品到购物车中，一个是获取购物车数据，实现一个添加商品到购物车的action，如下：

![1699925667637](./assets/1699925667637.png)

在商品详情页面，就添加商品到购物车，如下：

![1699925748139](./assets/1699925748139.png)

实现addCart方法，如下 ：

![1699926303166](./assets/1699926303166.png)

也就是说，还需要定义一个acction，这个action是用于修改购物车中某个商品的数量的，如下：

![1699926526928](./assets/1699926526928.png)

然后就可以在浏览器中测试了，如下：

![1699926850893](./assets/1699926850893.png)

看一下Pinia，如下：

![1699926890283](./assets/1699926890283.png)

目前，仓库中购物车中的商品的数量都是1，如下：

![1699926989436](./assets/1699926989436.png)

浏览器测试之，如下：

![1699927153345](./assets/1699927153345.png)

多点几次加入购物车，如下：

![1699927200400](./assets/1699927200400.png)

目前，购物车中就有数据了，但是一刷新，购物车中的数据就没有了，如下：

![1699927317968](./assets/1699927317968.png)

回到购物车组件，就直接去调用接口了，如下：

![1699927505100](./assets/1699927505100.png)

测试如下：

![1699927489992](./assets/1699927489992.png)
...
let remove = (cartItemId) => {
  store.removeShopCartItem(cartItemId);
};
</script>
...
const useCartStore = defineStore("Cart", () => {
    ...
    // 点击添加商品到购物车
    async function addShopCartItem(goodsId, goodsCount = 1) {
        await API.addNewCart(goodsId, goodsCount);
        getShopCartList();
    }

    // 修改购物车中某个商品的数量
    async function setShopCartItem(cartItemId, goodsCount) {
        await API.setCartCount(cartItemId, goodsCount);
        // 上面一行代码调用了接口，修改了某个商品的数量，那么list中的数据也要更新
        list.value.find(item => item.cartItemId === cartItemId).goodsCount = goodsCount;
    }
    ...
    async function removeShopCartItem(cartItemId) {
        ...
        // 上面一行代码调用了接口，修改了某个商品的数量，那么list中的数据也要更新
        list.value = list.value.filter(item => item.cartItemId != cartItemId)
    }
    ...
    const badge = computed(() => {
        const current = list.value.reduce((prev, next) => ((prev += next.goodsCount), prev), 0)
        return current
    })
...
![1699929831291](./assets/1699929831291.png)
