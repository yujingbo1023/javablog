...
![1715562953077](./assets/1715562953077.png)
...
关于vue.js的版本（老项目一般都是`Vue2`的，而新项目一般都选择`Vue3`开发）
    ...
    <div id="app">
        ...
        <div>
            ...
            <div>
                <label>年龄：</label>
                <input type="text" v-model="age" />
                <input type="button" :value="title" @click="addUserOrEditUser">
            </div>
        </div>
        <h3 class="penal">数据列表</h3>
        <table class="table">
            ...
            <tbody v-else>
                <tr>
                    <td colspan="3" style="text-align: center;">购物车空空如也~</td>
                </tr>
            </tbody>
        </table>
    </div>
...
```html
    <!-- 
        id="box" title="ok" class="item"  叫html标签内置的属性
        <h1 id="box" title="ok" class="item"></h1>


        ok="123" bad="456" id="xx" title="yy" 叫自定义属性
        ok="123"  123是字符串
        <ityls ok="123" bad="456" id="xx" title="yy"></ityls> 组件


        :id="xx" :title="yy" 叫自定义属性  
        xx 和 yy是data中定义的状态
        <ityls :id="xx" :title="yy"></ityls> 组件
        

        onclick="alert(11)" 叫原生DOM中的事件绑定
        <button onclick="alert(11)"></button> 

        bindtap="fn" 小程序中的事件绑定
        <button bindtap="fn"></button> 

        @click="alert(11)" vue中的事件绑定
        <button @click="alert(11)"></button> 

        @eat="fn" 自定义事件  事件名是eat  当eat事件发生了，会执行fn
        eat事件需要写代码去触发
        <ityls @eat="fn"></ityls> 

        @click="fn" 也叫自定义事件
        <ityls @click="fn"></ityls>  组件
...
* 什么是组件化？你是如何理解组件？
  + 现在主流的开发框，都是基于组件化的，组件的作用就是为了实现UI和逻辑的复用，实现声明式编程，在vue组件中，提供了很多的选项，如data, computed, watch, method, components, filter... 可以方便我们实现业务逻辑，组件化的好处就是可以复用UI和逻辑，在业务开发中，一般都会合理的划分组件，在vue中组件化的核心就是**自定义属性**，**自定义事件**，**自定义插槽**。组件与组件通信常见的有8种方案，比如：... 展开讲   如果看过源码，可以从源码角度分析组件的实现过程。
...
<img src="https://www.webwss.cn/usr/uploads/2022/06/4208038777.png" style="zoom: 80%; ">
...
<body>
  <!-- 
      vue的核心：
         1）数据驱动视图（响应式原理）
         2）Vue2是选项式API，有：el,data,methods,computed,watch,components,生命周期函数（8）,动态组件相关的（2个）
         3）组件化开发（自定义属性，自定义事件，自定义插槽，三大核心的目的是为了实现组件通信）

      一个Vue项目，有N个组件组件，通常一个项目只有一个根组件。有很多组件，可以有相同的逻辑，我们要想复用这些相同的逻辑，在Vue
      中，叫复用技巧，今天学习的混入，就是一种复用技巧。

      所谓的混入，就是向组件中混入一些可以复用的选项，目的是为了解决复用问题，混入分两类：
      - 1）全局混入：所有的组件都拥有混入的选项， 通过Vue.mixin({methods:{fn(){...}}})
      - 2）局部混入：通过一个mixin选项实现局部混入， 

      A B C{mixin:{fn()}} D{mixin:{fn()}}
     -->
  <div id="app">
    <h1>{{name}} -- {{a}} -- {{b}} -- {{fn()}}</h1>
    <hr>
...
* 在Vue2技术栈 中：Vue(2) + VueRouter(3) + Vuex(3)
...
![1715929891789](assets/1715929891789.png)
...
链接：http://47.99.134.126:5000/#/login
