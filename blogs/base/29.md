...
![1715307571058](./assets/1715307571058.png)
...
![1715323082954](./assets/1715323082954.png)



绑定点击事件：

![1715322927941](./assets/1715322927941.png)



实现checkedItem方法，如下：

![1715323499576](./assets/1715323499576.png)
...
checkedItem(event){
    ...
    if (!this.data.isEditCart) {   // 没有处于编辑模式下
      util.request(api.CartChecked, { productIds: this.data.cartGoods[itemIndex].productId, isChecked: this.data.cartGoods[itemIndex].checked ? 0 : 1 }, "POST", "application/json").then(res=>{
        if (res.errno === 0) {
          this.setData({
            cartGoods: res.data.cartList,
            cartTotal: res.data.cartTotal
...
![1715323737149](./assets/1715323737149.png)



定义一个方法，获取这个数量：

![1715323807808](./assets/1715323807808.png)
...
![1715324094105](./assets/1715324094105.png)

![1715324154019](./assets/1715324154019.png)



当点击全选时，也会使用到，然后开始实现全选功能，绑定点击事件，如下：

![1715324285893](./assets/1715324285893.png)



实现方法，如下：

![1715324851901](./assets/1715324851901.png)
  ...
  checkedAll(){
    if (!this.data.isEditCart) {
      ...
      let productIds = this.data.cartGoods.map(function (v) {
        return v.productId;   // 获取所有商品的ids
      });
      util.request(api.CartChecked, { productIds: productIds.join(','), isChecked: this.isCheckedAll() ? 0 : 1 }, "POST", "application/json").then(res=>{
        if (res.errno === 0) {
          this.setData({
            cartGoods: res.data.cartList,
            cartTotal: res.data.cartTotal
...
![1715325548457](./assets/1715325548457.png)

![1715325613499](./assets/1715325613499.png)
...
接下来，实现删除已选择的商品，绑定事件如下：

![1715325712290](./assets/1715325712290.png)



使用deleteCart方法，如下：

![1715326057639](./assets/1715326057639.png)
...
![1715326142542](./assets/1715326142542.png)



实现方法：

![1715326285839](./assets/1715326285839.png)
...
![1715326378102](./assets/1715326378102.png)



![1715326423093](./assets/1715326423093.png)
