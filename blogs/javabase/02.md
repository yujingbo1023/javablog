...
```java
// 通过class来定义一个类  在一个类中可以有多个方法
public class HelloWorld {
    public static void main(String[] args) {
        // 函数的调用  也是一个值   add(3)也是一个值，这个值是函数的返回值
        // 以后，你看到一个函数调用，在你眼就是一个值，什么值，是函数的返回值
        // add(3)  3叫实际参数，简称为实参
        // 函数调用的过程，就是实参向形参赋值的过程
        System.out.println(add(3)); // mysql  sum  avg
    ...
    // 叫函数/方法的声明
    // public static 不要管，后面学习
    // int 表示方法的返回值
    // return 666;  谁去调用add方法，就返回666
    // add 函数名 或 叫方法名 也需要满足标识符的命名规范
    // int n n叫形式参数，简称为形参，就当成函数内部的局部变量
    // 谁去调用add函数，就需要给形参赋值
...
public class HelloWorld {
    public static void main(String[] args) {
        System.out.println("欢迎使用计算机监控系统");
        // 下面的代码是伪代码
        if("CPU占用率">90%){
            send_email();
        }
...
![1719279849041](./assets/1719279849041.png)
...
public class HelloWorld {
    public static void main(String[] args) {
        // 函数调用的过程就是实参给形参赋值的过程
        // 现在给函数传递的数据是int类型，是基本类型
        int res = add(10, 20);
        System.out.println(res);

        // 一个函数内部的局部变量，只能在函数内部使用，不能在函数外部使用
        // System.out.println(a);
        // System.out.println(b);

        // {20,10,44,26} 是堆区的数据  有一个地址0x123
        // 把这个地址存储到了栈区0x123  arr这个变量中保存的就是一个地址0x123
        int[] arr = {20,10,44,26};
        System.out.println(arr);
        // arr是实参  arr是一个数组，数组是存储在堆区的
        // 函数调用的过程就是实参向形参赋值的过程 arr是一个地址
        printArray(arr);
    }
    public static int add(int a, int b ){
        // 形参相当于是函数内部的局部变量，只能在函数内部使用
        return a + b;
    }
    ...
    public static void printArray(int[] myarr){
        if(myarr == null){
            ...
            return;  // return有两个作用:1)提供返回值  2)结束函数调用
        }
        System.out.print("[");
...
```java
// java是面向对象的编程语言
// 类文件中,可以书写1个或多个类,类中可以定义很多数据(属性)和方法
public class HelloWorld {
    public static void main(String[] args) {
        // 方法调用在你的眼就是一个值,什么值,返回值
...
public class HelloWorld {
    public static void main(String[] args) {
        int[] arr = {1,2,3,4,5};
        // 你传递的是一个地址
        // System.out.println(reverse(arr));  // {5,4,3,2,1}
        reverse(arr);
    }
...
public class HelloWorld {
    public static void main(String[] args) {
        // getAvgScore(6);
        System.out.println("当前选手得分是:"+getAvgScore(6));
    }
    public static double getAvgScore(int n){
        ...
        // System.out.println("avg:"+(sum-min-max)/(n-2)*1.0);

        return (sum-min-max)/(n-2)*1.0;
    }
}
```
...
public class HelloWorld {
    public static void main(String[] args) {
        System.out.println("加密后的结果是："+encrypt(1864));  // [1, 8, 6, 4]
    }
...
public class HelloWorld {
    ...
    public static void start(int[] moneys){
        ...
        for (int i = 0; i < moneys.length; i++) {
            System.out.println("用户输入任意键开始抽奖：");
            String msg = sc.next();  // 卡住了,需要等待用户输入内容后才会向下走

            while (true){  // 死循环
                int index = rd.nextInt(moneys.length);  // 0-4
